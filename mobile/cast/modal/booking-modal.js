...
let _baseTotalPrice = 0;      // 元の予約の受取金額
let _opPriceMap     = new Map(); // オプションラベル → 価格

const ADD_EXT_PRICE_PER_30MIN = 5000; // 追加延長の単価（30分あたり）
...
function _updatePriceCells() {
  const extAddedPrice = (_addedExtMinutes / 30) * ADD_EXT_PRICE_PER_30MIN;
  const opAddedPrice  = [...(_selectedItems ?? [])].reduce((s, l) => s + (_opPriceMap.get(l) ?? 0), 0);
  const total      = _baseTotalPrice + extAddedPrice + opAddedPrice;
  const salary     = Math.floor(total / 2);
  const fmt = n => `¥${n.toLocaleString()}`;
  const c1 = document.getElementById('modal-price-cell');
  const c2 = document.getElementById('modal-salary-cell');
  if (c1) c1.textContent = fmt(total);
  if (c2) c2.textContent = fmt(salary);
}
...
export function initBookingModal({ onStart, onEnd, onCancel } = {}) {
  ...
  // スワイプで閉じる（ハンドル・ヘッダーをドラッグ）
  const _dragTarget = _sheet.querySelector('.modal-sheet__header');
  let _swipeStartY = 0;
  ...
  _dragTarget.addEventListener('touchmove', e => {
    ...
    if (dy > 0) _sheet.style.transform = `translateY(${dy}px)`;
  }, { passive: true });
  _dragTarget.addEventListener('touchend', e => {
    const dy = e.changedTouches[0].clientY - _swipeStartY;
    _sheet.style.transition = '';
    _sheet.style.transform  = '';
    if (dy > 80) closeBookingModal();
  });
...
export function openBookingModal(booking, { disableStart = false } = {}) {
  ...
  const fmt = n => `¥${n.toLocaleString()}`;
  _baseTotalPrice = totalPrice;
  _opPriceMap     = new Map((OP_OPTIONS[booking.shopId] ?? []).map(op => [op.label, op.price]));
  ...
  const cells = [
    ...
    ['受取金額',     fmt(totalPrice),                                  'modal-price-cell'],
    ['キャスト給与', fmt(castSalary),                                  'modal-salary-cell'],
